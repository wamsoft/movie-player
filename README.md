...
これを継承して各アーキテクチャ用の `include/<spec>/MoviePlayer.h`に
実装クラスの定義を配置してあります。

`include/MoviePlayer.h` 内で `ifdef` してアーキテクチャごとの実装ヘッダを
`#include "include/<spec>/MoviePlayer.h"` しています。
適当に使用するコードの作りに合わせてこの辺の構造は変更してください。
