...
Android 向け実装の注意点として、`Enqueue` で渡される `data` ポインタは
AMediaCodec の output buffer の借り物です。再生完了まで保持し続けると
codec output slot が pinned のままになり、codec input 側まで backpressure
が伝搬して decoder pacing が壊れる (= 音だけ遅く聞こえる) 現象が起きる
ため、Android 向け sink 実装では `Enqueue` 時に PCM を内部にコピーして
`param` をすぐ consumed 通知へ流すことが推奨されます
(`test/android/app/src/main/cpp/AAudioSink.h` が参考実装)。
...
`test/android/Makefile` (gradlew のラッパ) も用意してあるので、コマンド
ラインからは以下で動作確認できます (Git Bash / MSYS2 / WSL 推奨)。

```
cd test/android
make build      # gradlew :app:assembleDebug
make install    # 端末/エミュレータにインストール
make run        # install + launcher 起動
make logcat     # adb logcat
```

テスト app の音声出力には `AAudioSink` (`test/android/app/src/main/cpp/
AAudioSink.h`) を最小実装として同梱しています。AAudio (API 26+) のみで
動作し、AAudio オープン時に実 rate / channel / format を検証して
ミスマッチがあれば logcat に出します。
